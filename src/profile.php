...
if (!$is_admin && $profile_id !== $current_user_id) {
    die("Akses ditolak: Yahaha...Lu kagak bisa liat profil pengguna lain");
}

$message = "";
$is_csrf_detected = false;
$edit_mode = isset($_GET["edit"]);
$can_edit = ($current_user_id == $profile_id) || $is_admin;

if (isset($_GET['update_bio'])) {
    $target_id   = $is_admin ? $profile_id : $current_user_id;
    $input_bio   = mysqli_real_escape_string($conn, $_GET["bio"]);
    ...
    if ($stmt = mysqli_prepare($conn, $sql)) {
        mysqli_stmt_bind_param($stmt, "si", $input_bio, $target_id);
    ...
    if ($csrf_flag !== '') {
        error_log("CSRF flag triggered for user {$target_id}: {$csrf_flag}");
    }

    header("Location: profile.php?user_id={$target_id}&bio_updated=success&new_bio=" . urlencode($input_bio));
    exit;
}
